...
def _scene_is_perroom(scene_sprites):
    ...
    Restricted to entities whose y fits below the parking sentinel (v79; was
    "x and y both <= 255" up to v78).

    Per-room parks off-room actors at ss_y = 0xFF and every draw guard skips a
    sprite whose ss_y is >= 0xEF.  So the restriction that matters is about y,
    and about 0xEF rather than 0xFF:
      ...
      * x is no longer considered at all.  Up to v78 any entity past x=255
        ...
        quietly shared one scene.  Measured 2026-08-13 (#14 Step 1) that parking
        DOES survive a wide 16-bit build — the C draw loops compare a u16 ss_y,
        and ai_asm.s tests the high byte before comparing the low one against
