...
#elif defined(BW_SCENE_PERROOM)
/* Per-room scene instances without doors (degenerate but valid): current_bg is
 * still needed by scene_set_active_bg.  It stays 0 — with no doors the player
 * never leaves room 0. */
unsigned char current_bg;
#endif

#ifdef BW_SCENE_PERROOM
static void scene_set_active_bg(unsigned char n);
#endif
...
#ifdef BW_SCENE_PERROOM
    /* v75 — swap the active entity set to this room's. */
    scene_set_active_bg(n);
#endif
}
#endif
...
#ifdef BW_SCENE_PERROOM
/* v75 — per-room scene instances.  Activate only room `n`'s entities: restore
 * each of that room's entities to its authored home position, and park every
 * other room's entity fully off-screen at ss_y = 0xFF (the same "defeated"
 * sentinel the C AI bodies already skip on, `ss_y < 0xEF`).  Called at boot and
 * on every door transition, so a room only ever shows — and only ever collides
 * with — the entities placed in it; re-entering a room respawns its entities at
 * home (classic NES room behaviour).  ss_room / ss_home_x / ss_home_y come from
 * scene.inc, emitted only for multi-room projects.  The server also drives such
 * projects through the C scene draw + AI loops (NES_ASM_SCENE/AI off) so the
 * BW_SCENE_PERROOM draw-skip below drops parked actors and the C AI's own
 * ss_y<0xEF guard keeps them parked. */
static void scene_set_active_bg(unsigned char n) {
    unsigned char k;
    for (k = 0; k < NUM_STATIC_SPRITES; k++) {
        if (ss_room[k] == n) {
            ss_x[k] = ss_home_x[k];
            ss_y[k] = ss_home_y[k];
        } else {
...
#ifdef BW_SCENE_PERROOM
    /* v75 — activate the starting room's entities (the //@ init insert above sets
     * current_bg to the start room; default 0).  Parks every other room's entity
     * off-screen so only this room's actors are live from frame one. */
    scene_set_active_bg(current_bg);
#endif
