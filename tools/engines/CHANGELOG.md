...
This is `main`'s **v77**, taken whole — `builder-modules.js` and `studio-style.js` were
byte-identical to its parent here, so they applied unchanged. It ships as **v81** because
77, 78, 79 and 80 already name different engines on this branch (see
[`docs/plans/current/2026-08-27-port-forward-main-engine.md`](../../docs/plans/current/2026-08-27-port-forward-main-engine.md)),
and **the `targetEngine >=` gate had to move with the number**. Left at 77 it would emit
this feature into a project stamped 78, 79 or 80 — engines whose snapshots do not contain
it. Proved rather than reasoned: with the gate restored to 77, `enemy-bump.mjs`'s degrade
case fails with *"v80 target still emitted bw_bump*"*. Its ROM cases and
`tools/studio-tests/enemy-bump.spec.js` (2 passed) both run green here.
...
byte-for-byte as it did under v80. A design authored with the box ticked also
degrades cleanly on a pre-v81 target (`targetEngine >= 81` gate), matching how
