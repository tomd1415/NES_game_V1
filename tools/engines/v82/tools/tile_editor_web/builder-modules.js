...
(function () {
  ...
  modules['scene'] = {
    ...
    applyToTemplate(template, node, state) {
      ...
      // #30 / engine v81 — enemy-vs-enemy separation.  Lives on the Globals
      ...
      // pupil ticks it, and degraded on pre-v81 targets, so every existing
      ...
      // #30 / engine v81 — enemy-vs-enemy separation (AABB).  Needs two
      ...
      if (enemyBump && bumpIdx.length >= 2) {
        ...
        withHelper = A.appendToSlot(withHelper, 'declarations', [
          '/* [builder] #30 (engine v81) — enemy-vs-enemy separation state.',
        ...
        const bumpPass = [
          '        // [builder] #30 (v81) — enemy-vs-enemy separation.  Runs AFTER every',
  ...
  modules['dialogue'] = {
    ...
    applyToTemplate(template, node, state) {
      ...
      // State for the bounded, flash-free banner writer (feedback #31).  The
      // long "why" lives in the comment block below; the short version is that
      // the banner is now written one row per frame instead of one burst with
      // rendering switched off.
      const bannerState = [
        '/* [builder] dialogue — bounded banner writes (feedback #31).',
        ' *',
        ' * The banner is a full-width band 4 tile rows tall (8 when a vertical',
        ' * scroll leaves it straddling two attribute rows).  Writing it in ONE',
        ' * vblank costs 128-256 $2007 pokes — far past the ~2273-cycle window —',
        ' * so the old code switched rendering off (PPU_MASK = 0) for the burst',
        ' * and back on after.  That blank window IS the "stage glitches for a',
        ' * split second when the box opens" pupils reported.',
        ' *',
        ' * Now the job is spread over frames.  per_frame PREPARES one row into',
        ' * bw_dlg_buf[] (rendering is on there, with ~27000 cycles going spare)',
        ' * and the vblank slot blits those 32 bytes as unrolled',
        ' * `lda bw_dlg_buf+N / sta $2007` stores — ~8 cycles each, ~260 for the',
        ' * whole row, comfortably inside vblank.  Same prepare-then-burst trick',
        ' * scroll_stream() uses (see scroll.c).  Rendering is never disabled, so',
        ' * there is no flash.  A box takes 5 frames to appear (4 rows + the',
        ' * attribute step) instead of 1; at 60Hz that is under a tenth of a',
        ' * second, and the part-drawn band is invisible because a blank box cell',
        ' * (0x20) is colour 0 = the shared universal_bg in every palette.',
        ' *',
        ' * The band geometry is snapshotted when the job starts, so a camera that',
        ' * moves mid-animation cannot tear the band across two positions, and B',
        ' * is ignored until the job finishes so a mashed button cannot interleave',
        ' * a draw with a clear.',
        ' *',
        ' * The attribute bytes go FIRST on open (so glyphs never appear in the',
        ' * scenery palette) and LAST on close (so scenery never appears in the',
        ' * box palette). */',
        'static unsigned char bw_dlg_buf[32];   /* one prepared banner row */',
        'static unsigned int  bw_dlg_addr;      /* PPU address of bw_dlg_buf[0] */',
        'static unsigned char bw_dlg_job;       /* 0 idle, 1 drawing, 2 clearing */',
        'static unsigned char bw_dlg_step;      /* 0..bw_dlg_rows — rows + 1 attr step */',
        'static unsigned char bw_dlg_rows;      /* tile rows in the band (4 or 8) */',
        'static unsigned char bw_dlg_pend;      /* 0 nothing, 1 tiles, 2 attributes */',
        'static unsigned int  bw_dlg_twr0;      /* world tile row of the first text line */',
        'static unsigned char bw_dlg_total;     /* text rows this job draws */',
        'static unsigned char bw_dlg_alo;       /* first attribute row the band spans */',
        'static unsigned char bw_dlg_ahi;       /* last attribute row the band spans  */',
        '#ifdef SCROLL_BUILD',
        'static unsigned int  bw_dlg_wcol0;     /* camera tile column, snapshotted */',
        'static unsigned int  bw_dlg_addr2;     /* PPU address after the nametable wrap */',
        'static unsigned char bw_dlg_n1;        /* bytes before the wrap (32 = none) */',
        '#endif',
      ].join('\n');
      template = A.appendToSlot(template, 'declarations', [
      ...
      ].concat(lineDecls).concat([bannerState]).join('\n'));
      ...
      const perFrame = [
        ...
        '            /* #31: the banner now takes a few frames to paint.  While a',
        '             * job is in flight, swallow the button and hold the',
        '             * auto-close timer — otherwise a mashed B could queue a',
        '             * clear on top of a half-drawn box and leave the band',
        '             * stamped into the background. */',
        '            unsigned char dlg_busy = (bw_dlg_job != 0) || (bw_dialog_cmd != 0);',
        '            bw_dialog_prev_b = pad;',
        '            if (dlg_busy) b_edge = 0;',
        '            if (bw_dialog_open) {',
        ...
        '            }',
        '        }',
        '        // [builder] dialogue — prepare ONE banner row (#31).',
        '        // Rendering is on here, so building the row costs nothing the',
        '        // player can see; the vblank slot below only pushes bytes.',
        '        if (bw_dlg_pend == 0) {',
        '            if (bw_dlg_job == 0 && bw_dialog_cmd != 0) {',
        '                /* Start of job — snapshot the band geometry once so a',
        '                 * camera that moves mid-animation cannot tear it. */',
        '                unsigned int  dlg_twr0;',
        '                unsigned char dlg_total = BW_DIALOG_ROW_COUNT;',
        '#if BW_DIALOG_PER_NPC',
        '                /* A single-line per-NPC override still fills and restores',
        '                 * the full band, so only the DRAW is shortened. */',
        '                if (bw_dialog_cmd == 1',
        '                        && bw_dialogue_per_npc[bw_dialog_npc_idx] != 0) dlg_total = 1;',
        '#endif',
        '#ifdef SCROLL_BUILD',
        '                /* Camera-relative: the box tracks the frozen camera so it',
        '                 * stays on-screen on a scrolling map (pauseOnOpen holds',
        '                 * cam_x/cam_y still while the box is up). */',
        '                dlg_twr0 = (cam_y >> 3) + BW_DIALOG_ROW;',
        '                bw_dlg_wcol0 = (cam_x >> 3);',
        '#else',
        '                dlg_twr0 = BW_DIALOG_ROW;',
        '#endif',
        '                bw_dlg_twr0 = dlg_twr0;',
        '                bw_dlg_total = dlg_total;',
        '                /* Snap the band to the 4-tile attribute grid so whole',
        '                 * attribute bytes can be written (no read-modify-write).',
        '                 * alo..ahi is 1 row in the common case, 2 only under a',
        '                 * misaligned vertical scroll. */',
        '                bw_dlg_alo = (unsigned char)(dlg_twr0 >> 2);',
        '                bw_dlg_ahi = (unsigned char)((dlg_twr0 + dlg_total - 1) >> 2);',
        '                bw_dlg_rows = (unsigned char)((bw_dlg_ahi - bw_dlg_alo + 1) << 2);',
        '                bw_dlg_job = bw_dialog_cmd;',
        '                bw_dlg_step = 0;',
        '                bw_dialog_cmd = 0;',
        '            }',
        '            if (bw_dlg_job != 0) {',
        '                /* Step 0..bw_dlg_rows.  The attribute step is FIRST on an',
        '                 * open and LAST on a close; the others are one tile row',
        '                 * each, top to bottom. */',
        '                if (bw_dlg_step == ((bw_dlg_job == 1) ? 0 : bw_dlg_rows)) {',
        '                    bw_dlg_pend = 2;',
        '                } else {',
        '                    unsigned char dlg_j;',
        '                    unsigned char dlg_ri =',
        '                        (bw_dlg_job == 1) ? (bw_dlg_step - 1) : bw_dlg_step;',
        '                    unsigned int  dlg_wr = ((unsigned int)bw_dlg_alo << 2) + dlg_ri;',
        '                    unsigned char dlg_is_text = (dlg_wr >= bw_dlg_twr0)',
        '                        && (dlg_wr < bw_dlg_twr0 + bw_dlg_total);',
        ...
        '                    unsigned int dlg_wcol0 = bw_dlg_wcol0;',
        '                    if (dlg_wr >= 30) dlg_vbase = 0x2800 + (dlg_wr - 30) * 32;',
        '                    else              dlg_vbase = 0x2000 + dlg_wr * 32;',
        '                    dlg_src_base = dlg_wr * BG_WORLD_COLS + dlg_wcol0;',
        '                    /* The band is 32 tiles wide starting at the camera',
        '                     * column, so it can straddle the two side-by-side',
        '                     * nametables.  Precompute the split here rather than',
        '                     * testing every cell inside the vblank burst. */',
        '                    bw_dlg_addr = dlg_vbase + ((dlg_wcol0 & 32) ? 0x0400 : 0)',
        '                                + (dlg_wcol0 & 31);',
        '                    bw_dlg_n1 = (unsigned char)(32 - (dlg_wcol0 & 31));',
        '                    bw_dlg_addr2 = dlg_vbase',
        '                                 + (((dlg_wcol0 + bw_dlg_n1) & 32) ? 0x0400 : 0);',
        '#else',
        '                    dlg_src_base = dlg_wr * 32;',
        '                    bw_dlg_addr = 0x2000 + dlg_wr * 32;',
        '#endif',
        '                    if (bw_dlg_job == 1 && dlg_is_text) {',
        '#if BW_DIALOG_PER_NPC',
        '                        const unsigned char *npc_text =',
        '                            bw_dialogue_per_npc[bw_dialog_npc_idx];',
        '                        dlg_line = (npc_text != 0) ? npc_text',
        '                            : bw_dialogue_text_table[dlg_wr - bw_dlg_twr0];',
        '#else',
        '                        dlg_line = bw_dialogue_text_table[dlg_wr - bw_dlg_twr0];',
        '#endif',
        '                    }',
        ...
        '                            /* Column-compressed world: the tile at (row dlg_wr,',
        '                             * col dlg_wcol0+dlg_j) lives in the dedup table. */',
        '                            dlg_tile = bg_col_data[(unsigned int)',
        '                                bg_col_index[dlg_wcol0 + dlg_j] * BG_WORLD_ROWS',
        '                                + dlg_wr];',
        '#else',
      ...
      // Vblank writes: blit whatever per_frame prepared, inside the main
      // vblank window.  Bounded to one banner row (32 bytes) or one attribute
      // step per frame, which is what lets rendering stay ON — the old
      // unbounded burst had to force-blank, and that blank window was
      // feedback #31's "the stage glitches for a split second".
      const dlgUnrolled = Array.from({ length: 32 },
        (_, k) => '            PPU_DATA = bw_dlg_buf[' + k + '];');
      const vblank = [
        '        // [builder] dialogue — PPU writes (in vblank).',
        '        //',
        '        // Bounded work: at most one 32-byte banner row, or one attribute',
        '        // step, per frame.  per_frame above already built bw_dlg_buf[] and',
        '        // bw_dlg_addr, so all that happens here is pushing bytes — which is',
        '        // why rendering no longer has to be switched off (feedback #31).',
        '        //',
        '        // Restoring the row on close used to be trickier than this:',
        '        // earlier versions wrote tile 0x20 (space) which baked a',
        '        // visible stripe into the background, and the attempted fix',
        '        // tried to read the existing nametable out of VRAM via',
        '        // PPU_DATA — which broke in practice because of the',
        '        // buffered-read semantics (first read returns stale data;',
        '        // subsequent reads are one address behind) interacting badly',
        '        // with cc65 optimisations around the required dummy read.',
        '        //',
        ...
        '        if (bw_dlg_pend == 1) {',
        '#ifdef SCROLL_BUILD',
        '            /* Camera-relative band: it can straddle the two side-by-side',
        '             * nametables, so it goes out as up to two runs at the',
        '             * addresses per_frame worked out.  A scroll build already',
        '             * holds PPU_MASK at 0 across its whole vblank window (see the',
        '             * template), so the indexed loop is hidden there; what #31',
        '             * needed on this path was the burst bounded to ONE row, so',
        '             * that window stays a few scanlines rather than half a',
        '             * screen. */',
        '            {',
        '                unsigned char dlg_k;',
        '                PPU_ADDR = (unsigned char)(bw_dlg_addr >> 8);',
        '                PPU_ADDR = (unsigned char)(bw_dlg_addr & 0xFF);',
        '                for (dlg_k = 0; dlg_k < bw_dlg_n1; dlg_k++) PPU_DATA = bw_dlg_buf[dlg_k];',
        '                if (bw_dlg_n1 < 32) {',
        '                    PPU_ADDR = (unsigned char)(bw_dlg_addr2 >> 8);',
        '                    PPU_ADDR = (unsigned char)(bw_dlg_addr2 & 0xFF);',
        '                    for (; dlg_k < 32; dlg_k++) PPU_DATA = bw_dlg_buf[dlg_k];',
        '                }',
        '            }',
        '#else',
        '            /* Unrolled on purpose.  cc65 is invoked without -O, so',
        '             * `for (k = 0; k < 32; k++) PPU_DATA = bw_dlg_buf[k];` costs',
        '             * 50-65 cycles an iteration once the index promotion and',
        '             * cc65 stack juggling are counted — ~1800 for one row, whose',
        '             * tail would spill past vblank into the visible frame.  One',
        '             * statement per byte compiles to `lda bw_dlg_buf+N /',
        '             * sta $2007` (8 cycles) because the index is a compile-time',
        '             * constant: ~260 cycles for the row, comfortably inside',
        '             * budget with rendering left ON.  scroll_stream() unrolls its',
        '             * column burst for exactly the same reason. */',
        '            PPU_ADDR = (unsigned char)(bw_dlg_addr >> 8);',
        '            PPU_ADDR = (unsigned char)(bw_dlg_addr & 0xFF);',
      ].concat(dlgUnrolled).concat([
        '#endif',
        '            bw_dlg_pend = 0;',
        '            bw_dlg_step++;',
        '            if (bw_dlg_step > bw_dlg_rows) bw_dlg_job = 0;',
        '        } else if (bw_dlg_pend == 2) {',
        ...
        '             * Whole-byte writes — the band is attribute-grid aligned — so',
        '             * 8 of them in the common case, which is nothing.  Tiles AND',
        '             * attribute bytes are restored from the ROM-resident bg',
        '             * arrays, never read back from VRAM (see the round2-dialogue',
        '             * guards). */',
        '            unsigned char dlg_ar;',
        '            for (dlg_ar = bw_dlg_alo; dlg_ar <= bw_dlg_ahi; dlg_ar++) {',
        ...
        '                unsigned int dlg_wtc0 = bw_dlg_wcol0;',
