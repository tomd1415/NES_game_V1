  ...
  modules['dialogue'] = {
    ...
    applyToTemplate(template, node, state) {
      ...
      const vblank = [
        ...
        '            // Declared up here (not after the #if block below): cc65\'s',
        '            // default standard is C89, so a declaration must not follow',
        '            // a statement in the same block — and the per-NPC #if emits',
        '            // an `if (...)` statement before this point.',
        ...
        '#ifndef SCROLL_BUILD',
        '            /* Disable rendering for the VRAM burst.  On a non-scroll',
        '             * build nothing else holds PPU_MASK at 0, so a multi-row',
        '             * dialog (up to 3*28 PPU writes) that overruns vblank',
        '             * would corrupt the PPU render pointer and drop writes —',
        '             * the bug looks like only part of the text appearing.',
        '             * The SCROLL_BUILD path already clears PPU_MASK around',
        '             * the whole vblank window, so this is gated to non-scroll.',
        '             * Restored right after the loop, still inside vblank. */',
        '            PPU_MASK = 0;',
        '#endif',
        '            for (dlg_r = 0; dlg_r < dlg_total; dlg_r++) {',
        '                unsigned char dlg_srow = BW_DIALOG_ROW + dlg_r;',
        '                const unsigned char *dlg_line;',
        '                unsigned int dlg_vbase;     /* PPU base: nametable + row*32 */',
        '                unsigned int dlg_src_base;  /* index into the bg restore source */',
        '#ifdef SCROLL_BUILD',
        '                /* Anchor the box to the CURRENT camera so it stays on the',
        '                 * visible screen on a scrolling map.  pauseOnOpen freezes the',
        '                 * camera while the box is up, so cam_x/cam_y hold still.',
        '                 * cam_x/cam_y are world pixels; the horizontal nametable flips',
        '                 * at world col 32 (+0x400), the vertical at world row 30',
        '                 * (+0x800).  Without this the text wrote to fixed NT0 coords',
        '                 * and scrolled off-screen on multi-screen backgrounds. */',
        '                unsigned int dlg_wrow = (cam_y >> 3) + dlg_srow;',
        '                unsigned int dlg_wcol0 = (cam_x >> 3) + BW_DIALOG_COL;',
        '                if (dlg_wrow >= 30) dlg_vbase = 0x2800 + (dlg_wrow - 30) * 32;',
        '                else                dlg_vbase = 0x2000 + dlg_wrow * 32;',
        '                dlg_src_base = dlg_wrow * BG_WORLD_COLS + dlg_wcol0;',
        '#else',
        '                dlg_vbase = 0x2000 + (unsigned int)dlg_srow * 32;',
        '                dlg_src_base = (unsigned int)dlg_srow * 32 + BW_DIALOG_COL;',
        '#endif',
        '                if (bw_dialog_cmd == 1) {',
        '#if BW_DIALOG_PER_NPC',
        ...
        '#else',
        '                    dlg_line = bw_dialogue_text_table[dlg_r];',
        '#endif',
        '                }',
        ...
        '                        if (dlg_tile == 0) break;   /* end of text */',
        '                    } else {',
        '#ifdef SCROLL_BUILD',
        ...
        '                    }',
        '#ifdef SCROLL_BUILD',
        '                    /* (Re)point PPU_ADDR at the start and whenever the run',
        '                     * crosses a 32-tile nametable column boundary (the box',
        '                     * can straddle two side-by-side screens). */',
        ...
        '#else',
        '                    if (dlg_j == 0) {',
        '                        unsigned int dlg_addr = dlg_vbase + BW_DIALOG_COL;',
        '                        PPU_ADDR = (unsigned char)(dlg_addr >> 8);',
        '                        PPU_ADDR = (unsigned char)(dlg_addr & 0xFF);',
        '                    }',
        '#endif',
        ...
        '            }',
        '#ifndef SCROLL_BUILD',
        '            /* Re-enable rendering (base mask) now the VRAM burst is',
        '             * done but we are still inside vblank.  0x1E matches the',
        '             * engine base; a win/death tint reasserts itself in the',
        '             * next per-frame pass. */',
        '            PPU_MASK = 0x1E;',
        '#endif',
        '            bw_dialog_cmd = 0;',
