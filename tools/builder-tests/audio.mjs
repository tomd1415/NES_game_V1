...
`;
  ...
  // Case 10 — newer FamiStudio export shape (2026-04-27 audio
  // robustness pass).  Newer FamiStudio versions wrap their
  // `.export _<sym>=<sym>` lines in `.if FAMISTUDIO_CFG_C_BINDINGS`
  // ... `.endif`.  Bare ca65 errors with "Constant expression
  // expected" on that, blocking the build.  The server now
  // prepends a `FAMISTUDIO_CFG_C_BINDINGS = 0` definition (via
  // `_stage_audio_asm`) before assembly so the wrapped exports
  // are skipped cleanly.  This case exercises that fix end-to-end:
  // we hand the server a synthetic upload that uses the newer
  // shape and assert the build succeeds.
  const NEWER_FAMISTUDIO_SONG_ASM = `
; This file is for the FamiStudio Sound Engine and was generated by FamiStudio
.if FAMISTUDIO_CFG_C_BINDINGS
.export _music_data_pupil_song=music_data_pupil_song
.endif
.export _audio_default_music:=music_data_pupil_song
.export audio_default_music:=music_data_pupil_song
music_data_pupil_song:
\t.byte 1
\t.word @instruments
\t.word @samples-4
\t.word @song0ch0
...
\t.byte 6, 6, 6, 6, $00
`;
  const newerRom = await build({
    audioSongsAsm: NEWER_FAMISTUDIO_SONG_ASM,
    audioSfxAsm:   STUB_SFX_ASM,
  });
  if (sha1(newerRom) === stockHash) {
    fail('newer-FamiStudio export build matches stock — server prelude did not engage');
  }
  console.log('✓ newer-FamiStudio `.if FAMISTUDIO_CFG_C_BINDINGS`-wrapped export builds (server prelude rescues it)');

  // Case 11 — same shape with sfx omitted, to prove the asymmetric
  // upload path (server auto-stubbing missing sfx) also handles
  // newer-FamiStudio exports.  Pre-2026-04-27 this would have
  // either dropped audio entirely (asymmetric gate) or failed
  // assembly (FAMISTUDIO_CFG_C_BINDINGS).  Both fixed.
  const songOnlyNewerRom = await build({
    audioSongsAsm: NEWER_FAMISTUDIO_SONG_ASM,
  });
  if (sha1(songOnlyNewerRom) === stockHash) {
    fail('song-only newer-FamiStudio build matches stock — auto-sfx-stub or prelude did not engage');
  }
  console.log('✓ song-only newer-FamiStudio export builds (auto-sfx + prelude both engaged)');
