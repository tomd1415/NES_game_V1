// Engine v9 — SMB OAM flicker: codegen + cc65 compile. Gated on the SMB game
// type + engine v9; pre-v9 / non-smb / off emits nothing (golden-ROM safe).
...
function makeState() {
  ...
  const s = {
    name: 'smb-render', version: 1, universal_bg: 0x21,
    sprites: [{ role: 'player', name: 'hero', width: 2, height: 2, cells: mkCells(2, 2) },
      { role: 'enemy', name: 'E', width: 2, height: 2, cells: mkCells(2, 2) }],
  ...
  m.smbrender.enabled = true;
  m.scene.config.instances = [{ id: 1, spriteIdx: 1, x: 96, y: 200, ai: 'goomba', speed: 1 }];
  return s;
}

{
  const out = window.BuilderAssembler.assemble(makeState(), tpl);
  if (!/#define BW_OAM_FLICKER 1/.test(out)) { console.error('FAIL: v9 did not emit BW_OAM_FLICKER'); process.exit(1); }
  globalThis.NES_TARGET_ENGINE = 8;
  if (/#define BW_OAM_FLICKER/.test(window.BuilderAssembler.assemble(makeState(), tpl))) {
    console.error('FAIL: engine v8 target emitted BW_OAM_FLICKER'); process.exit(1);
  }
  globalThis.NES_TARGET_ENGINE = 9;
  const off = makeState(); off.builder.modules.smbrender.enabled = false;
  if (/#define BW_OAM_FLICKER/.test(window.BuilderAssembler.assemble(off, tpl))) {
    console.error('FAIL: flicker emitted while off'); process.exit(1);
  }
  console.log('✓ engine v9 emits OAM flicker only when on (SMB + v9)');
}
...
try {
  ...
  const r = await (await fetch(`http://127.0.0.1:${PORT}/play`, {
    ...
    body: JSON.stringify({ state: s, playerSpriteIdx: 0, playerStart: { x: 60, y: 120 },
      sceneSprites: [], mode: 'browser', customMainC: out, targetEngine: 9 }),
  })).json();
  if (!r.ok) { console.error('FAIL compile: smb-render rejected:', r.stage); console.error((r.log || '').slice(-2500)); process.exit(2); }
  console.log('✓ OAM flicker build compiles via cc65 (' + r.size + ' bytes, engine v' + r.engineVersion + ')');
} finally {
  srv.kill('SIGTERM');
  await sleep(300);
}
console.log('\nSMB rendering (engine v9) smoke-test complete.');
