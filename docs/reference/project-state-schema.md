...
2026-07-14 to the current `ENGINE_VERSION` = **75**) against
...
| `state.engineVersion` | **75** (advisory — tracks `NES_ENGINE_VERSION`) | Which C/ROM **engine** the design was authored for — provenance only | Stamped on new Studio projects from `NES_ENGINE_VERSION` (`studio-starter.js:349`); updated by the Studio "Update this game" advisor (`studio.js:1082`). |
...
- **Studio** loads `engine-version.js` (`studio.html:491`) → `NES_TARGET_ENGINE = NES_ENGINE_VERSION = 75` → targets the **latest** engine.
