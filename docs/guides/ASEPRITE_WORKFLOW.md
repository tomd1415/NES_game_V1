...
The project already ships **`tools/png2chr.py`** — you do **not** need to create
it. It converts an indexed-colour PNG (≤ 4 colours; width and height multiples
of 8) into NES planar CHR:

```bash
# Make a new CHR file:
python3 tools/png2chr.py input.png output.chr

# …or insert tiles into an existing CHR at a tile offset
# (--into requires --offset, so it can never silently overwrite from tile 0):
python3 tools/png2chr.py input.png output.chr --offset 0x40 --into existing.chr
```

It shares the planar 2-bit codec with the other CHR tools via
`tools/chr_codec.py`, so its output matches `generate_chr.py` exactly.
