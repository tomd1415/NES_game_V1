...
static unsigned char npc_x = 40, npc_y = 192;
...
static const unsigned char msg_hello[] = {
    0x17, 0x14, 0x1B, 0x1B, 0x1E, 0x34,
    0x17, 0x14, 0x21, 0x1E, 0x35, 0x00
};
...
static void write_palettes(void) {
    ppu_seek(0x3F, 0x00);
    PPU_DATA = 0x21; PPU_DATA = 0x29; PPU_DATA = 0x19; PPU_DATA = 0x07;
    ppu_seek(0x3F, 0x05);
    ...
    ppu_seek(0x3F, 0x1D);
    PPU_DATA = 0x16; PPU_DATA = 0x36; PPU_DATA = 0x06;
}
...
static void clear_text_row(unsigned char row, unsigned char col, unsigned char width) {
    unsigned char i;
    set_cell(row, col);
    for (i = 0; i < width; ++i) PPU_DATA = 0x00;
}

static void draw_text(unsigned char row, unsigned char col, const unsigned char *text) {
    unsigned char i = 0;
    set_cell(row, col);
    while (text[i] != 0x00) {
        PPU_DATA = text[i];
...
static void show_dialogue(const unsigned char *text) {
    waitvsync();
    PPU_MASK = 0;
...
static void update_playing(unsigned char a_pressed) {
    if (overlap(x, y, 16, 32, npc_x - 4, npc_y, 24, 16) && a_pressed) {
...
static void draw_npc(void) {
    draw_sprite(npc_y, 0x60, 0x02, npc_x);
    draw_sprite(npc_y, 0x61, 0x02, npc_x + 8);
    draw_sprite(npc_y + 8, 0x70, 0x02, npc_x);
    draw_sprite(npc_y + 8, 0x71, 0x02, npc_x + 8);
}
