...
class NativeShellTests(unittest.TestCase):
    ...
    def test_shell_constructs_without_starting_event_loop(self) -> None:
        ...
        self.assertTrue(window.quest_panel.isHidden())
        self.assertIsNotNone(window.chars_editor.widget())
        self.assertEqual(window._tile_library_buttons[0].width(), 38)
        window.resize(1440, 900)
        window._update_responsive_chrome()
        application.processEvents()
        self.assertFalse(window.quest_panel.isHidden())
        self.assertEqual(window._tile_library_buttons[0].width(), 42)
        ...
        window.findChild(object, "editSpritePixelsButton").click()
        self.assertEqual(window.editor_stack.currentWidget(), window.tile_editor)
        self.assertEqual(window.tile_bank.currentData(), "sprite")
        window.select_mode("CHARS")
        ...
        window.animation_frame_index.setValue(0)
        window.animation_frame_right.click()
        self.assertEqual(window._document.state["animations"][0]["frames"], [window.sprite_list.currentRow(), window.sprite_list.currentRow()])
        ...
        self.assertTrue(window.context_dock.isHidden())
        self.assertIsNotNone(window.rules_editor.widget())
        ...
        self.assertEqual(window.editor_stack.currentWidget(), window.code_editor)
        self.assertIn("#include", window.code_preview.toPlainText())
        self.assertTrue(window.code_c_button.isChecked())
        window.code_asm_button.click()
        self.assertTrue(window.code_asm_button.isChecked())
        self.assertIn(".include", window.code_preview.toPlainText())
        window.code_preview.setPlainText("; edited assembly\n")
        self.assertEqual(window._document.custom_source("asm"), "; edited assembly\n")
        ...
        self.assertEqual(len(window._master_palette_buttons), 64)
        window.findChild(object, "nesMasterColour16").click()
        self.assertEqual(window._document.background_palette(0)[0], 0x16)
        self.assertEqual(window._document.palette_recent_colours()[0], 0x16)
        ...
        window.findChild(object, "tilePen3").click()
        window._tile_pixel_buttons[4 * 8 + 3].click()
        self.assertEqual(window._document.sprite_tile_pixels(7)[4][3], 3)
        self.assertEqual(window._document.background_tile_pixels(7)[4][3], 1)
        window.tile_name.setText("Hero boot")
        window._set_tile_name()
        self.assertEqual(window._document.state["sprite_tiles"][7]["name"], "Hero boot")
        window.tile_bank.setCurrentIndex(0)
        window.tile_selector.setValue(7)
