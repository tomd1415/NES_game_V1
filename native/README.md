...
1. ~~`test_baseline_manifest.py::test_manifest_tracks_the_live_engine_baseline`
   — `63 != 75`.~~ Fixed. `baseline-v63.json` is now `baseline.json`: the version
   lives in the file as data instead of in its name, so it no longer needs
   renaming at every engine bump. Its automated results were re-verified at the
   current commit rather than inherited, and the 2026-07-10 attended attestation
   is explicitly marked as applying to v63 and **not** re-attested since.
2. ~~`test_phase0_starter_fixtures.py::test_artifact_hashes_and_input_immutability_match`
   — `FileNotFoundError` on all seven fixtures.~~ Fixed. `.gitignore` now negates
   `native/tests/fixtures/**/*.nes`, and the seven baseline ROMs are committed, so
   the byte-identical-ROM assertion executes for the first time — confirmed by
   flipping one byte and watching it go red.

   The baselines are **generated by the code they check**, so the manifest says so
   in as many words: a pass means *unchanged since `source_commit`*, not *correct*.
   Three of the seven ROMs changed against the old v63 baseline; whether that is
   intended is still open. See
